...
- `TASK.md` 建议控制在 300 行以内；超过 400 行时应检查是否需要拆分，不能通过折叠标签伪装大体积内容。
...
5. 将根 Task 和迭代模板都安装到项目约定位置；只有 `TASK.md` 作为默认恢复入口。
...
项目文件和两个任务模板分别记录各自的 `source_path`。只有合并成功并完成验证后，才能更新 `base_commit`。
...
1. **确认范围**：确认要更新全局文件、项目文件、根 Task 模板和迭代模板中的哪些目标。
...
5. **读取本地规则链**：解析 `CODEX_HOME`、项目根目录和当前目录，读取实际生效的 `AGENTS.md`、`AGENTS.override.md` 及两个任务模板。
...
```text
请按照 https://github.com/wuwenbin970731/AGENTS 的 README 协议，检查并提议同步全局 AGENTS、当前项目 AGENTS、TASK_TEMPLATE 和 TASK_ITERATION_TEMPLATE。
...
3. fetch 成功后，将 refs/remotes/origin/main 对应的完整 SHA 固定为本次 theirs 并报告；从该提交读取 README 和四个模板。不要使用本地 HEAD、未刷新的远端引用或 base_commit 作为最新远端版本，比较过程中不要悄悄切换提交。
...
```text
请按照当前项目 AGENTS.md、tasks/TASK_TEMPLATE.md 和 tasks/TASK_ITERATION_TEMPLATE.md，检查旧 Task `<旧 Task 路径>` 是否应迁移为目录式 Task 包。

本次只处理这一份旧 Task，不扫描后批量迁移其他文档。先进行只读检查并输出迁移提案，不要立即修改文件：

1. 读取旧 Task、当前适用的 AGENTS 规则和两个 Task 模板；检查项目根目录、Git 状态、旧 Task 的状态、行数、已有 Fxxx 编号、引用关系和目标是否仍然有效。
...
- 修改后检查四个模板之间的路径、状态名、必需章节和同步协议是否一致。
...
请先检查当前分支、工作区状态、README.md、GLOBAL_AGENTS.md、PROJECT_AGENTS.md、TASK_TEMPLATE.md 和 TASK_ITERATION_TEMPLATE.md，说明需求会影响哪些文件和规则。随后按最小改动原则实施，并遵守：
...
5. 修改后检查 Markdown 结构，并交叉检查四个模板之间的路径、状态名、必需章节、同步元数据和 README 协议是否一致；运行 git diff --check，确认没有敏感信息和无关文件。
